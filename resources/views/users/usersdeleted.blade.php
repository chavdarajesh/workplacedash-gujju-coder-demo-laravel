...
<a class="btn btn-primary mb-3" href="{{route('users')}}" role="button"><i class="fa fa-list" aria-hidden="true"></i> {{__('Users')}}</a>
@endif
    ...
    <tr class="trhover">      
      ...
      <td class="text-center">        
        @if($cuser->can('User Delete'))  
          <a href="{{ route('usersrestore',['id'=>$value->id]) }}" class="ml-3" onclick="return confirm('{{__('Are you sure you want to restore this user.')}}')"><i class="fa fa-trash fa-trash-restore-alt"></i></a>
        @endif  
      </td>
    </tr>
    <?php }?>
