...
<a class="btn btn-primary mb-3" href="{{route('universaluserscreate')}}" role="button"><i class="fa fa-plus" aria-hidden="true"></i> {{__('New')}}</a>
@endif
<table class="table gcspmaintable" cellpadding="5" cellspacing="0">
  ...
  <tbody>
    ...
    <tr class="trhover">      
      <td align="left"><a href="javascript:void(0);"><b>{{$value->name}}</b><span class="d-flex">{{$value->r_name}} / {{$value->email}}
        </span></a></td>
      <td class="text-center" >{{($value->email_verified_at!='')?__('Yes'):__('No')}}
        @if($value->email_verified_at=='')
        <a class="gcspresendlink" href="<?php echo  route('universalresendvarification',array('id'=>$value->id)); ?>">Resend</a>
        @endif
      </td>
      ...
      <td class="text-center">
        @if($cuser->can('User Edit'))
          <a href="{{ route('universalusersedit',['id'=>$value->id]) }}" class="ml-3"><i class="fa fa-edit"></i></a>
        @endif
        @if($cuser->can('User Delete'))  
          <a href="{{ route('universalusersdelete',['id'=>$value->id]) }}" class="ml-3" onclick="return confirm('{{__('Are you sure you want to delete this user.')}}')"><i class="fa fa-trash"></i></a>
        @endif  
      </td>
    </tr>
    <?php }?>
